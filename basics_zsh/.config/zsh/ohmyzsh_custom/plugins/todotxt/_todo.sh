...
case $state in
  (commands)
  cmdlist=(
    "add:add TODO ITEM to todo.txtx."
    ...
    "birdseye:display summary grouped by projects, contexts and complete projects."
    ...
    "schedule:set, modify or remove date threashold of an item."
  )
...
case $nextstate in
  ...
  (item)
  itemlist=(${${(M)${(f)"$(todo.sh -px list | sed '/^--/,$d')"}##<-> *}/(#b)(<->) (*)/${match[1]}:${match[2]}})
