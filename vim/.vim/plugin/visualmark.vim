...
if &bg == "dark"
 highlight SignColor ctermfg=black ctermbg=lightblue guifg=white guibg=RoyalBlue
else
 highlight SignColor ctermbg=white ctermfg=blue guibg=grey guifg=RoyalBlue
endif
hi SignLine cterm=underline,bold
" ---------------------------------------------------------------------
"  Public Interface:
if !hasmapto('<Plug>Vm_toggle_sign')
  map <silent> <F5> <Plug>Vm_toggle_sign 
endif
...
if !hasmapto('<Plug>Vm_goto_prev_sign')
  map <unique> <F7> <Plug>Vm_goto_prev_sign
endif
nnoremap <silent> <script> <Plug>Vm_goto_prev_sign	:call Vm_goto_prev_sign()<cr>

if !hasmapto('<Plug>Vm_remove_all_signs')
  map <unique> <unique> <S-F5>  <Plug>Vm_remove_all_signs
endif
nnoremap <silent> <script> <Plug>Vm_remove_all_signs	:silent! exe 'sign unplace *'<cr>
...
fun! s:Vm_place_sign()
  ...
  exe 'sign define SignSymbol text=>> linehl=SignLine texthl=SignColor'
